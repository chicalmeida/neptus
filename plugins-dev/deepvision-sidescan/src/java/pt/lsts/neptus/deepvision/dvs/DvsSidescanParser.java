...
public class DvsSidescanParser implements SidescanParser {
    ...
    public DvsSidescanParser(File f) {
        parser = new DvsParser(f);
    }
    ...
    public ArrayList<Integer> getSubsystemList() {
        return parser.getIndex().subSystemsList; //perguntar
    }
    ...
    public ArrayList<SidescanLine> getLinesBetween(long timestamp1, long timestamp2, int subsystem, SidescanParameters params) {
        ...
        while(ping.get(0).getTimestamp() < timestamp2) {
            DvsSonarData sboard = null;
            DvsSonarData pboard = null;
...
//            }
            ...
            for (DvsSonarData temp : ping) {
                if(temp != null) {
                    if (temp.getHeader().getChannel() == 0) { //perguntar
                        pboard = temp;
                    }
